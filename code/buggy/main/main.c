...
#define UDP_SERVER_IP "192.168.1.46"    //peter's laptop
//#define UDP_SERVER_IP "192.168.1.35"    //jareds laptop
//#define UDP_SERVER_IP "192.168.1.2"   //sourav's laptop
...
#define ECHO_PIN_2 GPIO_NUM_19

float final_mag, x_acl, y_acl, left_dist, right_dist, x_velocity, y_velocity, velocity;
int16_t front_dist;
...
void getAccel(float * xp, float *yp, float *zp) {
    ...
    *zp = read16_ACL(ADXL343_REG_DATAZ0) * ADXL343_MG2G_MULTIPLIER * SENSORS_GRAVITY_STANDARD;
    ...
    //printf("Magnitude: %f, X_acl: %.2f, Y_acl: %.2f\n", mag, *xp, *yp);
    final_mag = mag;
    x_acl = *xp;
    y_acl = *yp;
}
...
void init() {
    ...
    esp_rom_gpio_pad_select_gpio(TRIG_PIN_2);
    gpio_set_direction(TRIG_PIN_2, GPIO_MODE_OUTPUT);
    esp_rom_gpio_pad_select_gpio(ECHO_PIN_2);
...
float measure_distance() {
    ...
    int64_t start1_time = esp_timer_get_time();
    while(gpio_get_level(ECHO_PIN) == 1);  // Wait for echo to go low
    int64_t end1_time = esp_timer_get_time();

    // Calculate distance
    int64_t time1_difference = end1_time - start1_time;
    float distance = (time1_difference / 58.0); // Convert time to distance (in cm)
...
float measure_distance_2() {
    ...
    while(gpio_get_level(ECHO_PIN_2) == 0);  // Wait for echo to go high
    int64_t start2_time = esp_timer_get_time();
    while(gpio_get_level(ECHO_PIN_2) == 1);  // Wait for echo to go low
    int64_t end2_time = esp_timer_get_time();

    // Calculate distance
    int64_t time2_difference = end2_time - start2_time;
    float distance_2 = (time2_difference / 58.0); // Convert time to distance (in cm)
...
void hc_sr04_task1(void *pvParameters) {
    while (1) {
        float distance = measure_distance();
        right_dist = distance;
        //printf("Right_Side: %.2f cm\n", distance);
        vTaskDelay(120 / portTICK_PERIOD_MS);
    }
}

void hc_sr04_task2(void *pvParameters) {
    while (1) {
        float distance_2 = measure_distance_2();
        left_dist = distance_2;
        //printf("Left_side: %.2f cm\n", distance_2);
        vTaskDelay(120 / portTICK_PERIOD_MS);
    }
}
...
void print_task(void *pvParameters) {
    while (1) {
        //all data
        //sprintf(message,"%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d\n",speed, heading, velocity,final_mag, x_acl, y_acl, left_dist, right_dist, (int)front_dist); //servo speed set,steer_heading,velocity,acceleration (mag),x accel., y accel., left ultrasonic, right ultrasonic, front lidar
        
        //turn left-train
        //sprintf(message,"%d,%.2f,%.2f,%.2d,%d\n",speed, left_dist, right_dist, front_dist, heading); //servo speed set,steer_heading,velocity,left ultrasonic, right ultrasonic, front lidar
        
        //turn left-real
        sprintf(message,"%d,%.2f,%.2f,%.2d",speed, left_dist, right_dist, front_dist);
        //turn right
        //sprintf(message,"%d,%d,%.2f,%.2f,%.2f,%d\n",speed, heading, velocity, left_dist, right_dist, (int)front_dist); //servo speed set,steer_heading,velocity,left ultrasonic, right ultrasonic, front lidar
...
void get_velocity(void *pvParameters) {
    ...
    while (1) {
        /*
        x_velocity = x_velocity + x_acl;
        y_velocity = y_velocity + y_acl;
        if ((x_velocity > 0) && (y_velocity ))
...
void udp_client_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (len < 0) {
            ...
            else if (rx_buffer[0] == ':') { //heading from model
                strNum = rx_buffer + 1;
                heading = atoi(strNum);
                printf("%d\n",heading);
                //command_flag = 6;
...
void app_main() {
    ...
    init();
    printf("HC-SR04 sensor initialization complete.\n");
    i2c_master_init();
    i2c_scanner();
    /*
    uint8_t deviceID;
    ...
    */
    ...
    xTaskCreate(hc_sr04_task1, "hc_sr04_task1", 2048, NULL, 4, NULL);
    xTaskCreate(hc_sr04_task2, "hc_sr04_task2", 2048, NULL, 4, NULL);
    xTaskCreate(&sensorReadTask, "sensor_read_task", 2048, NULL, 5, NULL);
    //xTaskCreate(test_adxl343, "test_adxl343", 4096, NULL, 4, NULL);
    //xTaskCreate(get_velocity, "get_velocity", 2048, NULL, 5, NULL);
